...
async function fetchAll(){
    ...
    const infos = await Promise.all(list.map(async (item)=> {
        const res = await fetch("/api/info", {method: "POST", body: JSON.stringify({idx: item}), headers:{"Content-Type": "application/json"}});
        if (!res.ok) throw new Error('failed to fetch list');
        return res.json();
    }));
